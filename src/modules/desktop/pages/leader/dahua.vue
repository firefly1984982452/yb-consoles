      ...
      <el-col :span="16" class="top-left">
        ...
        <div class="approval">
          ...
          <main class="approval-main">
            <ul>
              <li v-for="(item, i) in approvalItemValue" :class="{ deving: item.itemValue == '开发中' }" :key="i" @click="handleApprove(item.title)">
                <p>{{ item.itemValue }}</p>
                <p>{{ item.title }}</p>
              </li>
            </ul>
      ...
      <el-col :span="16" class="temp1">
        <div class="watch">
          ...
          <main class="watch-main">
            <ul>
              <li @click="handleApprove('contract')">
                <p>{{ contractItemValue }}</p>
                <p>合同延长</p>
              </li>
              <li @click="handleApprove('care')">
                <p>{{ careItemValue }}</p>
                <p>照护评估</p>
              </li>
              <li @click="handleApprove('long')">
                <p>{{ longItemValue }}</p>
                <p>长护险评估</p>
              </li>
            ...
            <!-- <ul>
              ...
              <li style="visibility: hidden;" @click="handleApprove('开发中')">
                <p style="font-size:24px;">开发中</p>
                <p>员工合同</p>
              </li>
              <li style="visibility: hidden;" @click="handleApprove('开发中')">
                <p style="font-size:24px;">开发中</p>
            ...
            <template v-if="warnings.length > 0">
              <div v-for="(warning, i) in warnings" :key="i" class="has-content" @click="
                ...
                <span :class="{
                    ...
                    i_purple:
                      warning.messageFrom == 2004 || warning.messageFrom == 2003
                  }"></span>
                <span>
...
import LeaveCheck from "./dialog/leave-check"; // 离院审批--新的
...
export default {
  ...
  filters: {
    ...
    moneyFilter: function (money) {
      if (money && money != null && money != '' && money != undefined && money != "*****") {
        money = String(money);
  ...
  methods: {
    ...
    async initData() {
      ...
      for (var i = 0; i < res.data.length; i++) {
        ...
        switch (title) {
          ...
          // 住养长者
          case "入住老人":
            this.todayDynamic[4].itemValue = res.data[i].itemValue;
            break;
          // 离院长者
          case "今日离院":
            this.todayDynamic[5].itemValue = res.data[i].itemValue;
            break;
          // 请假长者
          case "请假长者":
            this.todayDynamic[6].itemValue = res.data[i].itemValue;
            break;
          // 应收
          case "应收":
          ...
          case "长护险评估":
            ...
            break;
          case "账单审核":
            this.billCheckItemValue = res.data[i].itemValue1;
            break;
          case "账单收款":
    ...
    async getBillCheckData() {
      const res = await axios.fetch("financeServer", "/accountBill/workbench");
    ...
    async getBookingData() {
      ...
      const looked = res.data.visitBookingViews.filter(item => {
        return item.status === 2;
      });
      this.todayDynamic[0].itemValue = res.data.visit + "/" + res.data.book;
    },
    // 获取入院长者数据
    async getLiveData() {
      ...
      if (res.data.length > 0) {
        ...
        this.$refs.live_check.peoples = res.data;
        this.$refs.live_check.elderId = res.data[0].elderId;
        this.$refs.live.currentPeople = res.data[0]; //默认当前老人信息为第一个
        this.$refs.live_check.detail = false;
        this.getLiveProgress(res.data[0].elderId);
        this.$refs.live_check.dialog = true;
      } else {
    ...
    async getLeaveData() {
      ...
      if (res.data.length > 0) {
        ...
        this.$refs.leave_check.peoples = res.data;
        this.$refs.leave_check.elderId = res.data[0].elderId;
        this.getLeaveProgress(res.data[0].elderId).then(res => {
          this.$refs.leave_check.elderFlows = res.data.elderFlows
            ? res.data.elderFlows
            : [];
          if (
            res.data.elderFlows.length > 0 &&
            res.data.elderFlows[res.data.elderFlows.length - 1].status == 1
          ) {
            this.$refs.leave_check.finish = true;
          }
          this.$refs.leave_check.dialog = true;
        });
      } else {
        ...
        &-main {
          ...
          ul {
            ...
            li:nth-child(4) {
              ...
              // background: url("../../../../assets/images/bg4.png") no-repeat 0 0;
            }
            li:nth-child(5) {
              p:nth-child(1) {
                font-size: 24px;
              ...
              // background: url("../../../../assets/images/bg5.png") no-repeat 0 0;
            }
            li:nth-child(6) {
              p:nth-child(1) {
                font-size: 24px;
              ...
              // background: url("../../../../assets/images/bg6.png") no-repeat 0 0;
            }
          }
        }
      }
