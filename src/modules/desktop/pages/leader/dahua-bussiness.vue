        ...
        <div class="watch">
          <header class="header">
            <div class="left">
              <i></i>
              <span>寿星统计</span>
              <span class="total">{{oldData.length}} 位寿星</span>
            </div>
            <div class="right">
              ...
              <el-button type="primary" @click="exportToExcel" round>导出</el-button>
            </div>
          </header>
          <main class="watch-main">
            <div class="longevity" style="height:100%;">
              <el-table :data="oldData" stripe style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    //寿星数据导出
    exportToExcel() {
      dataToExcel(this.oldData, {
        keyValue: "姓名",
        stringValue2: "性别",
        stringValue: "出生日期",
        itemValue: "年龄",
      }, `长者生日_${this.birthMonth}.xlsx`);
    },
    sizeWatch() {
    ...
    async initData() {
      ...
      for (var i = 0; i < res.data.length; i++) {
        ...
        switch (title) {
          ...
          // 约定变更
          case "今日变更":
            this.todayDynamic[7].itemValue = res.data[i].itemValue;
            break;
          // 应收
          case "应收":
            this.billItemValue[0].itemValue = res.data[i].itemValue1;
            this.billItemValue[0].itemValue1 = res.data[i].itemValue;
            break;
          // 实收
          case "实收":
            this.billItemValue[1].itemValue = res.data[i].itemValue1;
            this.billItemValue[1].itemValue1 = res.data[i].itemValue;
            break;
          // 欠款
          case "欠费":
            this.billItemValue[2].itemValue = res.data[i].itemValue1;
            this.billItemValue[2].itemValue1 = res.data[i].itemValue;
            break;
          /**审批事项 */
          case "入院":
      ...
      switch (name) {
        ...
        case "contract":
          ...
          var routerUrl = this.$router.resolve({
            path: "/elder/contract?expireTime=30&status=100"
          });
        ...
        case "care":
          ...
          // }
          var routerUrl = this.$router.resolve({
            path: "/elder/care/assessList?expireTime=30"
          });
          window.open(routerUrl.href, "_blank");
          break;
        // 长护险
        case "long":
          ...
          // }
          var routerUrl = this.$router.resolve({
            path: "/elder/longNurseRisk?expireTime=60"
          });
          window.open(routerUrl.href, "_blank");
          break;
        default:
    ...
    .temp1 {
      ...
      .watch {
        ...
        .header {
          ...
          justify-content: space-between;
          i {
            background: url("../../../../assets/images/icon-work-birthday.png")
              no-repeat 100% 100%;
          }
          .total {
            color: #999;
