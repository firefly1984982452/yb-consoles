  ...
  <div>
    <el-form :inline="true" v-show="moreQuery" class="search">
      <el-form-item :label="item.name" v-for="item in allMenuList" :key="item.id">
        <el-select v-model="item.choose" multiple collapse-tags placeholder="请选择">
    ...
    <el-form :inline="true" v-show="moreQuery" class="search">
      <el-form-item label="类别">
        ...
        <el-button :type="currentMenuType == item.id ? 'primary' : ''" v-for="item in menuType" :key="item.id" @click="changeMenuType(item.id)">{{item.title}}</el-button>
      </el-form-item>
    </el-form>
    <div class="content">
      <el-row type="flex" class="pannel">
        ...
        <el-col :span="11">
          <span>区域：</span>
          <Area ref="areaCom" @currentArea="getArea" :area="setArea" @allAreas="getAllAreas" />
        </el-col>
        <el-col :span="1">
      ...
      <el-table class="ub-table" :data="data" row-key="id" :span-method="arraySpanMethod" border>
        <el-table-column prop="periodDate" label="日期">
          <template slot-scope="scope">
            <span>{{currentWeekData[scope.$index].text}}（{{currentWeekData[scope.$index].value}}）共：{{scope.row.count}}份</span>
          </template>
        </el-table-column>
        <el-table-column :label="week.categoryTitle" v-for="(week,point) in data[0].summaryTwoVoList" :key="point">
          <template slot-scope="scope">
            <div class="menu-content">
              <div class="tag-list-text" v-if="data[scope.$index].summaryTwoVoList.length > 0">
                <div class="tag" :key="tag" v-for="tag in data[scope.$index].summaryTwoVoList[point].summaryOneVoList">
                  <span class="name">{{tag.foodTitle}}：</span>
                  <span class="value">{{tag.count}}</span>
                </div>
              </div>
            </div>
          </template>
...
export default {
  data() {
    return {
      ...
      data: [{ summaryTwoVoList: [{ summaryOneVoList: [{ "foodId": 17, "foodTitle": "绿畦香稻粳米饭", "count": 2 }] }] }],
      menuType: [],
      menuCookType: [],
      allMenuList: [],
      currentWeekData: [{
  ...
  methods: {
    ...
    getAllAreas(val) {
      window.localStorage.setItem('allArea', JSON.stringify(val));
    },
    changeMenuType(val) {
    ...
    print(data) {
      ...
      let param = {
        ...
        menuCategoryId: this.search.title,
        foodIds: arr,
        areaId: this.search.area,
        areaName: this.search.areaName
      }
      this.openPrint({
        path: '/print/elder/menuCollect',
        query: {
          param: JSON.stringify(param),
          currentWeekData: JSON.stringify(this.currentWeekData)
        }
      });
    },
    // 点击周切换
    ...
    init() {
      ...
      let typeNet = new Promise((resolve, reject) => {
        axios.fetch("dietServer", "/dietCategory/query", { type: 1 }).then(res => {
          if (res.code == 200) {
            resolve(res.data);
          }
        })
