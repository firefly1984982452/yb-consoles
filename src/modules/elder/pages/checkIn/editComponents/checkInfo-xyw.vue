            ...
            <el-row :gutter="10">
              ...
              <el-col :xl="6" :lg="7" style="width:23.2%;">
                <el-form-item label="长护险评估等级" prop="insuranceLevel">
                  <el-select v-model="dict['长护险评估等级'].items.item0" placeholder="请选择长护险等级" @change="[dictUtils.onchange($event, 0, dict['长护险评估等级']),changeLevel]" value-key="itemValue" clearable :disabled="notEdit==true">
              ...
              <el-col :xl="6" :lg="7" style="width:30%;">
                <el-form-item label="长护险评估有效期" label-width="130px" v-show="showDate">
                  <el-date-picker v-model="insuranceDateRange[0]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="开始日期" clearable :disabled="notEdit==true"></el-date-picker>
                  <el-date-picker v-model="insuranceDateRange[1]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="结束日期" clearable :disabled="notEdit==true"></el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="0">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        checkinNo: [{ required: true, message: "", trigger: "blur,change" }],
        checkinDate: [{ required: true, message: "", trigger: "blur,change" }],
        bedId: [{ required: true, message: "", trigger: "blur,change" }],
        nursingLevel: [{ required: true, message: "", trigger: "blur,change" }],
        insuranceLevel: [
  ...
  watch: {
    dict: {
      handler(n, o) {
        this.showDate = this.dict["长护险评估等级"].values.value0 == 0 ? false : true;
      },
      deep: true
    }
  },
  ...
  methods: {
    ...
    infoInit() {
      ...
      axios.fetch("elderServer", "/elderInfo/all", data).then(res => {
        ...
        that.dict["长护险评估等级"].values.value0 = resData.insuranceLevel
          ? resData.insuranceLevel
          : 0;
        that.dictUtils.setValues(that.dict);
        that.insuranceDateRange = that.form.insuranceStart
          ? [that.form.insuranceStart, that.form.insuranceEnd]
          : [];
        if (that.form.bedId) {
          that.sign = "init";
        ...
        .then(res => {
          ...
          if (res.data.code == "200") {
            ...
            if (res.data.data.srsv_room_name) {
              srsv_floor_id = parseInt(
                res.data.data.srsv_room_name.split("")[0]
              );
            }
            that.form = {
              ...
              se_changhu_level: res.data.data.srsv_changhu_level
                ? res.data.data.srsv_changhu_level
                : ""
            };
            this.initSign = 1;
            if (res.data.data.srsv_jinji_yinshi) {
              that.yinshi = [].concat(
                res.data.data.srsv_jinji_yinshi.split(",")
              );
            }
            that.changebuilding(res.data.data.srsv_building_id);
            that.dict["院方护理等级"].values.value0 =
              res.data.data.srsv_nursing_level;
            that.dict["长护险评估等级"].values.value0 =
              res.data.data.srsv_changhu_level;
    ...
    save() {
      if (this.dict["长护险评估等级"].values.value0 === 0) {
        this.insuranceDateRange = [null, null];
      } else if (this.dict["长护险评估等级"].values.value0 >= 7 && (!this.insuranceDateRange[0] || !this.insuranceDateRange[1])) {
        let today = this.insuranceDateRange[0] ? this.insuranceDateRange[0] : this.$moment().format('YYYY-MM-DD');
        let end = this.insuranceDateRange[1] ? this.insuranceDateRange[1] : '2099-12-31'
        this.insuranceDateRange = [today, end];
      }
      if (!this.form.checkinNo) {
        return this.$message.error("请输入入院编号");
      }
      if (!this.form.checkinDate) {
        return this.$message.error("请选择入院日期");
      }
      if (!this.form.buildingId === "") {
        return this.$message.error("请选择楼宇名称");
      }
      if (!this.form.floor) {
        return this.$message.error("请选择楼层");
      }
      if (!this.form.roomId) {
        return this.$message.error("请选择房间号");
      }
      if (!this.form.bedId) {
        return this.$message.error("请选择床位号");
      }
      if (!this.dict["院方护理等级"].values.value0) {
        return this.$message.error("请选择院方护理等级");
      }
      if (this.dict["长护险评估等级"].values.value0 === null) {
        return this.$message.error("请选择长护险评估等级");
      }
      if (
        this.level.includes(this.dict["长护险评估等级"].values.value0.toString()) &&
        (!this.insuranceDateRange[0] ||
          !this.insuranceDateRange[1])
      ) {
        return this.$message.error("请选择长护险评估有效期");
      }
      this.form.nursingLevel = this.dict["院方护理等级"].values.value0;
      this.form.nursingLevelName = this.dict["院方护理等级"].lists.list0.filter(
        item => {
          return this.dict["院方护理等级"].values.value0 == item.itemValue;
        }
      )[0].title;
      this.form.insuranceLevel = this.dict["长护险评估等级"].values.value0;
      this.form.insuranceLevelName = this.dict[
        "长护险评估等级"
      ].lists.list0.filter(item => {
        return this.dict["长护险评估等级"].values.value0 == item.itemValue;
      })[0].title;

      this.form.startTime = this.insuranceDateRange[0];
      this.form.endTime = this.insuranceDateRange[1];
      this.form.insuranceStart = this.insuranceDateRange[0];
      this.form.insuranceEnd = this.insuranceDateRange[1];
      ...
      axios
        ...
        .then(res => {
          ...
          window.localStorage.setItem("nursingLevel", JSON.stringify(this.form.nursingLevel)); //对象转字符串
          window.localStorage.setItem("checkinDate", JSON.stringify(this.form.checkinDate)); //对象转字符串
          this.$emit(
            "controlNavClick",
            "FirstServer",
            null,
