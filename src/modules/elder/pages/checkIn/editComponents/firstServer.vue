		...
		methods:{
            ...
            saveAssemble(){
                ...
                for(let i=0; i<this.nursingServiceArr.length; i++){
                    for(let j=0; j<this.nursingServiceArr[i].content.length; j++){
                        this.nursingServiceArr[i].content[j].isDraw = this.nursingServiceArr[i].content[j].isDraw==true?1:0;
                    }
			...
			childAction1(){
				...
				.then(res => {
                    ...
                    this.$router.push('/elder/changeList');
				})
            },	
        },
        computed: {
