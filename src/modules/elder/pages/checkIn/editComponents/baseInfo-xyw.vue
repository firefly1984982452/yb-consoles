...
export default {
  ...
  created() {
    ...
    // 已经生成老人id的情况
    if (this.elderId) {
      this.infoInit(this.elderId, bookId);
    }
    // 从预定过来的并且没有生成老人id
    if (bookId && elderId == 0 && !this.elderId) {
  ...
  methods: {
    ...
    infoInit(elderId) {
      ...
      axios.fetch("elderServer", "/elderInfo/all", data).then(res => {
        ...
        const checkinDate = this.form.checkinDate ? this.form.checkinDate : "";
        this.$emit("getCheckInDate", checkinDate);
      });
    },
    change(e) {
      this.$forceUpdate();
    ...
    save() {
      this.$refs["theForm"].validate(valid => {
        ...
        if (!valid) {
          ...
          !/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(this.form.cardNo)
        ) {
