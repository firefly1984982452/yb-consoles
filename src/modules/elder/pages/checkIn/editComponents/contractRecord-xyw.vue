  ...
  methods: {
        ...
        if (res.data.length > 0) {
          res.data.forEach(item => {
            if (data.agreementId == item.agreementId) {
              // console.log(item);
              this.$refs.contractEditDialog.form = Object.assign(
                {},
                this.$refs.contractEditDialog.form,
                item
              );
    ...
    confirm(data) {
      this.$confirm("盖章确认后将无法修改, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await axios.fetch("elderServer", "/elder/elderById", {
            elderId: this.elderId
          });
          if (res.code == 200) {
            ...
            axios
              ...
              .then(res => {
                ...
                if (data.contractType == 0) {
                  this.$emit("changeNotEdit", true);
                  window.localStorage.setItem("notEdit", true);
                }
              });
          }
        })
