  ...
  <div>
      <el-card class="box-card" v-for="(item,index) in form" :key="index" style="margin-bottom:10px;">
        <div slot="header" class="clearfix clearBtns" style="height: 35px">
          ...
          <el-button @click="addFamily(item)" class="addBtn">
            <i class="iconfont iconjichutubiao-tianjia-huise"></i>新增家属
          </el-button>
          <el-button @click="editFamily(index)" class="addBtn" style="background:#6cbc6c;margin-top: 0;:hover{background:#6cbc6c}!important">
            <i class="iconfont iconcaozuotubiao-bianji"></i>编辑家属
          </el-button>
        </div>
        <!-- 查看时 -->
            ...
            <el-row :gutter="10">
              ...
              <el-col :xl="21" :lg="21">
                ...
                <div class="buttons" style="margin-left:110px;">
                  <el-button type="primary" @click="save('saveForm')">保存</el-button>
...
export default {
  name:"FamilyInfo",
  components: { ubAddress, ubDictionary,ubAvatar },
  props: ["elderId",'familySaved'],
  data() {
    return {
      sign: "",
      ...
      showDash: 1,
      option: [],
      onlyRelation: true, // 是否为唯一监护人（第一次进入或只有一个家属时）
      form: [], //渲染表单
      //保存表单
      saveForm: {
        relationPhoto:'',
  ...
  methods: {
    ...
    editFamily(index){
      ...
      this.onlyRelation = index == 0 && this.saveForm.guarder == '是' ? true : false;
    },
    /*新增家属*/
    addFamily(val) {
      // 如果没编辑家属，直接点新增
    ...
    deleteFamily(index){
      ...
      axios
      .fetch('elderServer', "/elderRelation/delete", data)
      .then(res => {
    ...
    save(formName){
      ...
      that.$refs[formName][0].validate(valid => {
        ...
        .then(res => {
            ...
            that.$emit("familyinfo", "familyInfo", "careFor");
        })
      })
