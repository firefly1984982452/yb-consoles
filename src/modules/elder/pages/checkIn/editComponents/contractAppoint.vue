<template>
	<el-card class="box-card contractAppoint">
		<el-form :model="form" label-width="95px" :rules="rules">
			...
			<el-row>
				...
				<el-form-item label="" label-width="0" style="display:inline-block;">
					<el-date-picker v-model="serverDate[0]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="开始日期" clearable></el-date-picker>
					<el-date-picker v-model="serverDate[1]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="结束日期" clearable></el-date-picker>
				</el-form-item>
			</el-row>
			<el-row>
				...
				<el-form-item label="" label-width="0" style="display:inline-block;">
					<el-date-picker v-model="tryLiveDate[0]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="开始日期" clearable></el-date-picker>
					<el-date-picker v-model="tryLiveDate[1]" type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd" placeholder="结束日期" clearable></el-date-picker>
				</el-form-item>
			</el-row>
			<h4>入住费用</h4>
			<el-row>
				<el-col :span="3" class="width18" style="width:14%;">
					<el-form-item label="服务费" prop="serviceFee">
						<el-input v-model="form.serviceFee" style="width:100px;" maxlength="8"  ></el-input><span class="margin-left-word">元/月</span>
					</el-form-item>
				</el-col>
...
<script>
	...
	export default {
		props: ["elderId","elderName","checkinDate"],
		data() {
		...
		methods:{
			...
			getBaseFee(){
                ...
                axios
				.fetch('elderServer', "/elder/amount", data)
				.then(res => {
					let resData = res.data;
					this.form.bedFee = resData.filter(item=>{return item.category==="基础项"&&item.title==="托管费"})[0].feeAmount;
					this.form.careFee = resData.filter(item=>{return item.category==="基础项"&&item.title==="护理费"})[0].feeAmount;
					this.form.dietFee = resData.filter(item=>{return item.category==="基础项"&&item.title==="膳食费"})[0].feeAmountDay;
					this.form.depositFee = resData.filter(item=>{return item.category==="基础项"&&item.title==="保证金"})[0].feeAmount;
					this.form.serviceFee = parseFloat(this.form.bedFee) + parseFloat(this.form.careFee);
				})
			},
			initServiceDate(month){ 
				this.form.serviceStartDate = this.checkinDate;
				this.form.serviceEndDate = this.computeYmd(this.form.serviceStartDate,month);
			...
			initTryLiveTime(day){
				...
				this.tryLiveDate = [this.form.probationStartDate,this.form.probationEndDate];
			},
			computeYmd(val, n) {
				//根据时间和月份算出几月以后的日期
				let str = val.split('-');
		    ...
		    childAction(){
				if(this.form.serviceInterval === '' || this.form.serviceInterval === undefined){
		      		this.$message.error('请输入服务时间');return;
		      	}else if(this.serverDate.length === 0 || this.serverDate === undefined){
		      		this.$message.error('请输入服务时间范围');return;
		      	}else if(this.form.probationInterval === '' || this.form.probationInterval === undefined){
		      		this.$message.error('请输入试住期');return;
		      	}else if(this.tryLiveDate.length === 0 || this.tryLiveDate === undefined){
		      		this.$message.error('请输入试住期范围');return;
		      	}else if(this.form.serviceFee === '' || this.form.serviceFee === undefined){
		      		this.$message.error('请输入服务费');return;
		      	}else if(this.form.bedFee === '' || this.form.bedFee === undefined){
		      		this.$message.error('请输入床位费');return;
		      	...
		      	}else if(this.form.depositFee === '' || this.form.depositFee === undefined){
		      		this.$message.error('请输入入住保证金');return;
		      	}else if(this.form.agreeOut === '' || this.form.agreeOut === undefined){
		      		this.$message.error('请输入外出约定意见');return;
		      	}else if(this.form.agreeMedicine === '' || this.form.agreeMedicine === undefined){
		      		this.$message.error('请输入委托发药意见');return;
				}
