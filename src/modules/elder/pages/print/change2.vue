  ...
  <div>
    <div class="page confirm_table1">
      <p style="font-size:18px;margin-top:60px;">
        <span style="margin-right:1em;">附件3</span>变更事项确认表（1-3）
      </p>
      <h2 class="title">变更事项确认表（1）</h2>
      <table cellspacing="0" cellpadding="0" class="table">
        <tr>
          <td width="15%">姓名</td>
          <td width="12%">{{ changeInfo.elderName }}</td>
          <td width="11%">性别</td>
          <td width="11%">{{ changeInfo.gender }}</td>
          <td width="13%">出生年月</td>
        ...
        <tr>
          ...
          <td colspan="2">
            照护等级(首次)
            <span class="info2" v-if="changeInfo.levelName">{{
              changeInfo.levelName
            }}</span>
            <span class="info2" v-else></span>
            评估日期
            <span class="info" v-if="changeInfo.createTime">{{
              changeInfo.createTime
            }}</span>
        ...
        <tr>
          ...
          <td colspan="5" style="padding:0;">
            <table class="inner-table">
              <tr v-for="(item, i) in estimatePrintVos" :key="i">
                <td valign="top" style="position: relative;">
                  ...
                  <div style="position:absolute;bottom:2em;">
                    照护等级
                    <span class="info" v-if="item.currentDescription">{{
                ...
                <td>
                  ...
                  <div class="right">
                    <span
                      class="info"
                      style="width:150px;"
                      v-if="item.currentStart"
                      >{{ item.currentStart | dateFormat }}</span
                    >
                    <span v-else
                      >日期：<span class="info1"></span>年<span
                        class="info1"
                      ></span
                      >月<span class="info1"></span>日</span
                    >
                  </div>
                </td>
              </tr>
        ...
        <tr>
          ...
          <td colspan="6" style="padding:0;">
            <table class="inner-table">
              <tr v-for="(item, i) in printBedVos" :key="i">
                <td valign="top">
                  <div>
                    <p>
                      <b>第{{ i + 1 }}次变更</b>
                    ...
                    <div style="margin:2em 0;">
                      床位(托管)费：
                      <span class="info2" v-if="item.currentBedFee">{{
                        item.currentBedFee
                      }}</span>
                      <span class="info2" v-else></span>
                      元/月
                    ...
                    <div>
                      ...
                      <span
                        class="info2"
                        v-if="
                          printNursingVos.length > 0 &&
                            printNursingVos[i].currentNursingFee
                        "
                        >{{ printNursingVos[i].currentNursingFee }}</span
                      >
                      <span class="info2" v-else></span>
                      元/月
                    </div>
                ...
                <td>
                  ...
                  <div class="right">
                    <span
                      class="info"
                      style="width:150px;"
                      v-if="firstChange.bedTime"
                      >{{ firstChange.bedTime | dateFormat }}</span
                    >
                    <span v-else
                      >日期：<span class="info1"></span>年<span
                        class="info1"
    ...
    <div class="page confirm_table3">
      ...
      <table cellspacing="0" cellpadding="0" class="table">
        <tr>
          <td style="width:15%;text-align-last:justify;">姓名</td>
          <td style="width:12%">{{ changeInfo.elderName }}</td>
          <td style="width:11%">性别</td>
          <td style="width:11%">{{ changeInfo.gender }}</td>
          <td style="width:13%;text-align:center;">出生年月</td>
          <td style="width:38%">{{ changeInfo.birthDay | dateFormat }}</td>
        </tr>
        <tr>
          <td>入住区域</td>
          <td>{{ changeInfo.area }}</td>
        ...
        <tr>
          ...
          <td colspan="5" valign="top" style="padding:0;">
            <table class="inner-table">
              <tr v-for="(item, i) in agreeOutPrintVos" :key="i">
                <td>
                  <p>
                    <b>第{{ i + 1 }}次变更</b>
                  </p>
                  <div>
                    <p>
                      1.乙方
                      <span v-if="item.currentDescription">{{
                        item.currentDescription | outFilter
                      }}</span>
        ...
        <tr>
          ...
          <td colspan="5" valign="top" style="padding:0;">
            <table class="inner-table">
              <tr v-for="(item, i) in agreeMedicinePrintVos" :key="i">
                <td colspan="3" valign="top">
                  <p>
                    <b>第{{ i + 1 }}次变更</b>
                  </p>
                  <div>
                    <p>
                      1.乙方外出就医所配药品
                      <span class="info2" v-if="item.agreeMedicinePrintVos">{{
                        item.agreeMedicinePrintVos | medicineFilter
                      }}</span>
                      <span class="info2" v-else></span>
                      （应填写委托或不委托）甲方按医嘱发放
                    </p>
                    <p>
                      ...
                      <span
                        class="info2"
                        v-if="firstChange.currentDescription"
                        >{{
                          firstChange.currentDescription | agreeFilter
                        }}</span
                      >
                      <span class="info2" v-else></span>
                      （应填写接受或不接受）乙/丙方委托：外出就医所配药品按医嘱发放
                    </p>
                  </div>
                </td>
                <td colspan="2">
  ...
  methods: {
    ...
    changeData() {
      ...
      axios.fetch("elderServer", "/elderChange/print", data).then(res => {
        ...
        this.estimatePrintVos =
          res.data.estimatePrintVos.length > 0
            ? res.data.estimatePrintVos.reverse()
            : [{currentDescription:'',currentStart:''}];
        this.printBedVos = res.data.printBedVos.reverse();
        this.printNursingVos = res.data.printNursingVos.reverse();
        this.agreeOutPrintVos =
          res.data.agreeOutPrintVos.length > 0
            ? res.data.agreeOutPrintVos.reverse()
            : [
                {
                  currentDescription: "",
                  currentStart: ""
        ...
        this.agreeMedicinePrintVos =
          res.data.agreeMedicinePrintVos.length > 0
            ? res.data.agreeMedicinePrintVos.reverse()
            : [{ currentDescription: "", currentStart: "" }];
      });
    },
    //处理变更数据
