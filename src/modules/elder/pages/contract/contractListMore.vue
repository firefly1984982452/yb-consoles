...
<!-- 某老人的合同列表 -->
	<div>
		<div class="title"><el-button class="back" size="mini" @click="back">返回</el-button><strong>历史记录</strong></div>
		<el-card class="box-card contractRecord">
			...
			<div class="block" v-if="elderName">
				<el-timeline>
					<el-timeline-item v-for="(item, index) in tableData" :key="index" :color="item.isCurrent == true?'#F49B31':''" 
					size="large" placement="top">
						<p>
							<span>{{item.createDate}}</span>
							<span>{{item.createTime}}</span>
							<span v-if="item.isCurrent == true">使用中</span>
						</p>
						<el-card>
							<el-form label-width="70px">
								<el-row>
									...
									<el-col :span="2">
									<span class="detail_btn" @click="print(item)" :style="{'top':item.contractType===0?'37px':'20px'}">查看详情</span>
									</el-col>
								</el-row>
							</el-form>
						</el-card>
	...
	export default {
		...
		methods:{
			...
			dataInit(){
				...
				axios
				.fetch('elderServer', "/elder/elderById", data)
				.then(res => {
			...
			handleTable(){
			   ...
			   let d = nowDate.getDate()<10?"0"+nowDate.getDate():nowDate.getDate();//当是个位的时候补0
			   nowDate = nowDate.getFullYear()+'-'+m+'-'+d;
			   this.checkDay =  this.datedifference(this.tableData[0].checkinDate, nowDate);
			   this.checkYM = this.datedifYM(this.tableData[0].checkinDate, nowDate)
			   this.elderName = this.tableData[0].elderName;//老人姓名
			   this.checkinDate = this.tableData[0].checkinDate;//入住时间

			   //创建时间
			   this.tableData.map((item,index)=>{
				   item.createDate = item.createTime?this.dateFormat(item.createTime):'';
