<template>
  <div>
    ...
    <div class="content">
      <ul class="title">
        <span>合计：累计欠款 <b>{{amountDetail.num}}</b> 人，欠收金额 <b>{{amountDetail.amount}}</b> 元</span>
        <el-button @click="output" type="primary" icon="el-icon-download">数据导出</el-button>
      </ul>
      <el-table :data="data" border :header-cell-style="{background:'#F3F3F5'}" class="ub-table">
        ...
        <el-table-column prop="billTitle" label="账单名称" align="center"></el-table-column>
        <el-table-column prop="feeAmount" label="账单金额(元)" align="center"></el-table-column>
        <el-table-column prop="receivableAmount" label="欠款金额(元)" align="center"></el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      axios.fetch('financeServer', '/accountReceivable/owe/all', info).then(res => {
        if (res.code == 200) {
          this.data = res.data
          this.total = res.total
        }
      })
      axios.fetch('financeServer', '/accountReceivable/owe/statistics', info).then(res => {
        if (res.code == 200) {
      ...
      axios.fetch('financeServer', '/accountReceivable/owe/all', info).then(res => {
        if (res.code == 200) {
          ...
          dataToExcel(res.data, {
            ...
            billTitle: "账单名称",
            feeAmount: "账单金额(元)",
            receivableAmount: "欠费金额(元)",
          }, `养老机构欠费明细表.xlsx`, "养老机构欠费明细表");
